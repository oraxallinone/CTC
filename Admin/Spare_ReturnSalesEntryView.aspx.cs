...
public partial class Admin_Spare_PurchaseEntry : System.Web.UI.Page
{
    ...
    public void filldata(string sino, string VcNO)
    {
        ...
        var v = from c in db.AME_Spare_SalesEntryBillDetails.ToList().Where(t => t.Sp_Id == id && t.Sp_InvoiceNo == VouNo && t.Branch_Name == Session["Branch"].ToString()) select c;
    ...
    protected void txt_PartNo_TextChanged(object sender, EventArgs e)
    {
        try
        {
            if (btn_addd.Text == "Add")
            {
                txt_RetrnQty.Enabled = false;
    ...
    protected void imgbtn_PartDelete_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string VNo = Convert.ToString(Request.QueryString["No"]);
        string year2 = ViewState["jcyear"].ToString();
        AME_Spare_SalesEntry pe = db.AME_Spare_SalesEntry.ToList().First(t => t.Sp_InvoiceNo == VNo && t.Itm_Partno == PartNo && t.Ss_Status == "SE" && t.Branch_Name == Session["Branch"].ToString() && t.jc_year==year2);
        db.DeleteObject(pe);
        db.SaveChanges();
        FillGrid();
    }
    ...
    protected void btn_Submit_Click(object sender, EventArgs e)
    {
        ...
        if (txt_BName.ToolTip == "")
        {
            ...
            //txt_BName.Focus();
            Response.Redirect("Spare_ReturnSalesEntryView.aspx?id=" + sino + "&No=" + VNo);
            return;
        }
        ...
        db.SaveChanges();
        string year1 = ViewState["jcyear"].ToString();
        AME_Daily_SpareSales_Report chan = db.AME_Daily_SpareSales_Report.First(t => t.DR_InvoiceNo == VouNo && t.Branch_Name == Bname && t.jc_year==year1);

        chan.Dr_InvoiceTotal = Convert.ToDecimal(txt_AFinalAmount.Text);
        chan.Dr_Output13_5 = Convert.ToDecimal(txt_AVatAmount.Text);
        chan.Dr_Spare13_5 = Convert.ToDecimal(txt_ANetAmount.Text);
        db.SaveChanges();
    ...
    protected void txt_BName_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            txt_tooltip.Text = v.First().Mc_code;
            txt_PartNo.Focus();
        }
        catch
        {
    ...
    protected void txt_RetrnQty_TextChanged(object sender, EventArgs e)
    {
        ...
        if (btn_addd.Text != "Save")
        {
            ...
            decimal previousqty = Convert.ToDecimal(txt_PartQuantity.Text);
            decimal rtnqty = Convert.ToDecimal(txt_RetrnQty.Text);
            decimal avlqnty = previousqty - rtnqty;
            txt_PartAmount.Text = Convert.ToString(avlqnty * Convert.ToDecimal(txt_PartRate.Text));
            txt_PartTaxAmount.Text = (Convert.ToDecimal(txt_PartAmount.Text) * Convert.ToDecimal(txt_PartVat.Text) / 100).ToString("0.00");
            txt_PartTotal.Text = (Convert.ToDecimal(txt_PartAmount.Text) + Convert.ToDecimal(txt_PartDiscount.Text) + Convert.ToDecimal(txt_PartTaxAmount.Text)).ToString("0.00");
    ...
    {
        txt_RetrnQty.Enabled = true;
        Button imgEdit = (Button)sender;
        string pno = Convert.ToString(imgEdit.ToolTip);
        GridViewRow row = (GridViewRow)imgEdit.NamingContainer;
        //int rowind = row.RowIndex;
        Label deletelbl = (Label)row.FindControl("Label11");
        Label idlbl = (Label)row.FindControl("lbl_ss_ID");
        txt_SS_ID.Text = idlbl.Text;
        decimal returnno = 0;
        returnno = Convert.ToDecimal(deletelbl.Text);
        txt_PartQuantity.Text = returnno.ToString();
        string VNo = Convert.ToString(txt_BVoucherNo.Text);
        string year1 = ViewState["jcyear"].ToString();
        var details = (from c in db.AME_Spare_SalesEntry.ToList()
                       where c.Sp_InvoiceNo == VNo && c.Itm_Partno == pno && c.Ss_Status == "SE"
                       && c.Branch_Name == Session["Branch"].ToString() && c.jc_year==year1
                       select c);
        if (details.Count() > 0)
        {
    ...
    protected void btn_addd_Click(object sender, EventArgs e)
    {
        ...
        string year1 = ViewState["jcyear"].ToString();
        var ChkPrdCode = from c in db.AME_Spare_SalesEntry.Where(t => t.Itm_Partno == txt_PartNo.Text && t.Branch_Name == branch && t.Sp_InvoiceNo == InNo && t.jc_year==year1) select c;

        if (Convert.ToInt32(ChkPrdCode.Count()) > 0 && btn_addd.Text == "Add")
        {
        ...
        if (Convert.ToDecimal(txt_AvlQty.Text) < Convert.ToDecimal(txt_PartQuantity.Text) && btn_addd.Text == "Add")
        {
        ...
        if (txt_RetrnQty.Text == "0" && btn_addd.Text == "Save")
        {
            ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "MessagePopUp", "<script language='JavaScript'>alert('Enter a quantity to return...!!!'); </script>", false);
            txt_RetrnQty.Focus();
            return;
        }

        if (txt_PartQuantity.Text  == "0.00" && btn_addd.Text == "Save")
        {
            ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "MessagePopUp", "<script language='JavaScript'>alert('Enter a quantity to return...!!!'); </script>", false);
            txt_RetrnQty.Focus();
            return;
        ...
        //smitaDbAccess.insertprocedure("Sp_StockdispatchInSpareIssue1", CWParam1, CWParamValue1);
        ...
        string year4 = ViewState["jcyear"].ToString();
        AME_Spare_SalesEntry pr1 = db.AME_Spare_SalesEntry.First(t => t.Itm_Partno == txt_PartNo.Text && t.Sp_InvoiceNo == vcNo && t.Branch_Name == branch && t.jc_year==year4);

        pr1.Ss_Quantity = pr1.Ss_Quantity - Convert.ToDecimal(txt_RetrnQty.Text);
        ...
        AME_Spare_SaleReturn ass = new AME_Spare_SaleReturn();
        ass.Branch_Name = Session["Branch"].ToString();
        ass.Created_By = Session["Uid"].ToString();
       
        ass.Created_Date = SmitaClass.IndianTime();
        ass.Sp_InvoiceNo = Convert.ToString(txt_BVoucherNo.Text);
        ass.Sp_Id = sino;
        ass.Sp_PartyCode = txt_BName.ToolTip;
        ass.Itm_Partno = txt_PartNo.Text;
        ass.Itm_PartDescrption = txt_PartDesc.Text;
        ass.Ss_Quantity = Convert.ToDecimal(txt_PartQuantity.Text);
        ass.Ss_ReturnQuantity = Convert.ToDecimal(txt_RetrnQty.Text);
        ass.Ss_Rate = Convert.ToDecimal(txt_PartRate.Text);
        ass.Ss_Amount = Convert.ToDecimal(txt_PartAmount.Text);
        ass.Ss_Discount = Convert.ToDecimal(txt_PartDiscount.Text);
        ass.Ss_Vat = Convert.ToDecimal(txt_PartVat.Text);
        ass.Ss_TaxAmont = Convert.ToDecimal(txt_PartTaxAmount.Text);
        ass.Ss_Total = Convert.ToDecimal(txt_PartTotal.Text);
        ass.Sales_ReturnDate = Convert.ToDateTime(txt_returndate.Text, SmitaClass.dateformat());
       

        ass.Status = true;
        
        db.AddToAME_Spare_SaleReturn(ass);
        db.SaveChanges();
        FillGrid();
        ...
        //if (btn_addd.Text == "Save")
        //{
        //    var v=from x in db.AME_Spare_SalesEntry.ToList().Where(t => t.Sp_InvoiceNo == vcNo && t.Branch_Name == branch && t.Status==true)
        //              select x;
    ...
    {
        Response.Redirect("Spare_Return_SalesEntry.aspx");
    }
}
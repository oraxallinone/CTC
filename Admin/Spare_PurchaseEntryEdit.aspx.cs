...
public partial class Admin_Spare_PurchaseEntry : System.Web.UI.Page
{
    ...
    private void FillSlno()
    {
        ...
        var v = (from c in db.AME_Spare_PurchaseEntry.ToList()
    ...
    public void filldata(string sino, string VcNO)
    {
        ...
        string year = Request.QueryString["year"];
        var v = from c in db.AME_Spare_PurchaseEntryBillDetails.ToList().Where(t => t.Sp_Id == id && t.Sp_VoucherNo == VouNo && t.Branch_Name == Session["Branch"].ToString() && t.Status==true && t.jc_year==year) select c;
        ...
        var details = (from c in db.AME_Spare_PurchaseEntry.ToList()
                       where c.Sp_VoucherNo == VouNo && c.Ss_Status == "PE" && c.Branch_Name == Session["Branch"].ToString()
                       && c.jc_year==year
                       select c);
    ...
    protected void btn_PartAdd_Click(object sender, EventArgs e)
    {
        ...
        else if (btn_PartAdd.Text == "Update")
        {
            ...
            string year = Request.QueryString["year"];
            AME_Spare_PurchaseEntry pr1 = db.AME_Spare_PurchaseEntry.First(t => t.Itm_Partno == txt_PartNo.Text && t.Sp_VoucherNo == vcNo && t.Ss_Status=="PE" && t.jc_year==year);
            pr1.Ss_Quantity = Convert.ToDecimal(txt_PartQuantity.Text);
            pr1.Ss_NetQuantity = Convert.ToDecimal(txt_PartQuantity.Text);
            pr1.Ss_Rate = Convert.ToDecimal(txt_PartRate.Text);
            pr1.Ss_Amount = Convert.ToDecimal(txt_PartAmount.Text);
        ...
        AME_Spare_PurchaseEntryBillDetails pd = db.AME_Spare_PurchaseEntryBillDetails.First(t => t.Sp_Id == id && t.Sp_VoucherNo == VouNo && t.Branch_Name == Bname && t.jc_year==year1);
    ...
    private void FillGrid()
    {
        ...
        string year = Request.QueryString["year"];
        var details = (from c in db.AME_Spare_PurchaseEntry.ToList()
                       where c.Sp_VoucherNo == VNo && c.Ss_Status == "PE" && c.Status==true
                       && c.Branch_Name == Session["Branch"].ToString() && c.jc_year==year
                       select c);
        GridView2.DataSource = details.ToList();
        GridView2.DataBind();
    ...
    protected void imgbtn_PartDelete_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string year = Request.QueryString["year"];
        var details = (from c in db.AME_Spare_PurchaseEntry.ToList()
                       where c.Sp_VoucherNo == VNo && c.Itm_Partno == PartNo && c.Ss_Status == "PE"
                       && c.Branch_Name == Session["Branch"].ToString() && c.jc_year==year
                       select c);
        ...
        AME_Spare_PurchaseEntry pe = db.AME_Spare_PurchaseEntry.ToList().First(t => t.Sp_VoucherNo == VNo && t.Itm_Partno == PartNo && t.Ss_Status == "PE" && t.Branch_Name == Session["Branch"].ToString() && t.jc_year==year);
        db.DeleteObject(pe);
        db.SaveChanges();
        FillGrid();
    }
    ...
    protected void btn_Submit_Click(object sender, EventArgs e)
    {
        ...
        string Bname = Session["Branch"].ToString();
        string year = Request.QueryString["year"];
        AME_Spare_PurchaseEntryBillDetails pd = db.AME_Spare_PurchaseEntryBillDetails.First(t => t.Sp_Id == id && t.Sp_VoucherNo == VouNo && t.Branch_Name == Bname && t.jc_year==year);
    ...
    protected void ImgEdit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string year = Request.QueryString["year"];
        var details = (from c in db.AME_Spare_PurchaseEntry.ToList()
                       where c.Sp_VoucherNo == VNo && c.Itm_Partno == pno && c.Ss_Status == "PE" && c.Branch_Name==branchname
                    && c.jc_year==year   select c);
        if (Convert.ToDecimal(details.First().Ss_Quantity) != Convert.ToDecimal(details.First().Ss_NetQuantity))
        {
            ScriptManager.RegisterStartupScript(this.Page, typeof(string), "", "alert('Error..!! Stock Already Used Now You Cant Edit Stock..!!');", true);
            txt_PartNo.Text = "";

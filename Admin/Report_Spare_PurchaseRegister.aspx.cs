...
public partial class Report_ProjectExpensesDetailsCodeDatewise : System.Web.UI.Page
{
    ...
    protected void imgbtnview_Click(object sender, ImageClickEventArgs e)
    {
       ...
       // int sino = Convert.ToInt32(imgview.ToolTip);
        ...
        string sino = imgview.ToolTip;





        ScriptManager.RegisterStartupScript(Page, typeof(Page), "OpenWindow", "window.open('PurchaseEntryView.aspx?id=" + sino + "&year=" + imgview.CommandArgument + "');", true);
       
       // Response.Redirect("PurchaseEntryView.aspx?id=" + sino + "&year=" + imgview.CommandArgument);

        //var url = string.Format("PurchaseEntryView.aspx?id=" + sino + "&year=" + imgview.CommandArgument);

...
public partial class Report_ProjectExpensesDetailsCodeDatewise : System.Web.UI.Page
{
    ...
    public void FillGrid()
    {
        ...
        string param = "@Fromdate,@Todate,@Branch,@Suppliercode";
        string paramvalue = Convert.ToDateTime(txt_FromDate.Text, SmitaClass.dateformat()).ToString("yyyy-MM-dd HH:mm:ss") + " , " + Convert.ToDateTime(txt_ToDate.Text, SmitaClass.dateformat()).ToString("yyyy-MM-dd HH:mm:ss") + "," + Branch+","+ddl_BSuplier.SelectedValue;

        DataTable dtr = smitaDbAccess.SPReturnDataTable("sp_Spare_PurchaseListPartywise", param, paramvalue);
        if (dtr.Rows.Count > 0)
        {
    ...
    protected void btn_Show_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, typeof(string), "", "alert('Please Select Supplier Name..!!');", true);
                ddl_BSuplier.Focus();
                return;
            }

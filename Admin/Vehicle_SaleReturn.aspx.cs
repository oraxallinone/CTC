    ...
    public void filldata()
    {
        try
        {
            ...
            if (Convert.ToInt32(SalesList.Count()) > 0)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "MessagePopUp", "<script language='JavaScript'>alert('This Sale Already Return..!!'); </script>", false);
                    txt_billno.Focus();
                    txt_billno.Text = "";
                    btn_salereturn.Visible = false;
                    display.Style.Add(HtmlTextWriterStyle.Display, "none");
                    return;
                ...
                else
                {
                    ...
                    btn_salereturn.ToolTip = Convert.ToString(SalesEntry.First().Vs_Billno);
    ...
    protected void ddl_model_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        protected void btn_Submit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Session["billno"] = txt_billno.Text;
                Response.Redirect("Form21.aspx");
        ...
        protected void btn_salereturn_Click(object sender, EventArgs e)
        {
            if (txt_billno.Text == "")
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "MessagePopUp", "<script language='JavaScript'>alert('Bill No shouldnot bE blank..!!'); </script>", false);
            ...
            string billno = Convert.ToString(btn_salereturn.ToolTip);
            //vechicle salesEntry Update
            string branchname = Session["Branch"].ToString();
            AME_Vehicle_SaleEntry vpe = db.AME_Vehicle_SaleEntry.First(t => t.Vs_Billno == billno && t.Branch_Name == branchname);
            vpe.Status = "SalesReturn";
            vpe.Vse_Quantity = 0;
            db.SaveChanges();
            //vechicle sales entrydetails update
            AME_Vehicle_SaleEntryDetails sed = db.AME_Vehicle_SaleEntryDetails.First(t => t.Vs_Billno == billno && t.Branch_Name == branchname);
            sed.Vq_Status = "SalesReturn";
            db.SaveChanges();
            //vechicle sales retun insert
            AME_VehicleSales_Return vsr = new AME_VehicleSales_Return();
            vsr.Branch_Name = Session["Branch"].ToString();
            vsr.Created_By = Session["Uid"].ToString();
            vsr.Created_Date = SmitaClass.IndianTime();
            vsr.Mv_ModelName =Convert.ToInt32(ddl_model.SelectedValue);
            vsr.Mv_VehicleType = ddl_VType.SelectedItem.Text;
            vsr.Vp_Chassisno = ddl_chessisno.SelectedItem.Text;
            vsr.Vp_Engineno = txt_engineno.Text;
            vsr.Vp_Quantity =Convert.ToDecimal(txt_Quantity.Text);
            vsr.Status = "SaleReturn";
            vsr.Vr_returndate = Convert.ToDateTime(txt_returndate.Text,SmitaClass.dateformat());
            vsr.Vs_Billno = txt_billno.Text; 
            db.AddToAME_VehicleSales_Return(vsr);
            db.SaveChanges();
            //update vehicle purchase Entry 
            int modelname = Convert.ToInt32(ddl_model.SelectedValue);
            AME_Vehicle_PurchaseEntry pe = db.AME_Vehicle_PurchaseEntry.First(t=>t.Mv_VehicleType==ddl_VType.SelectedItem.Text
            && t.Mv_ModelName == modelname && t.Vp_Chassisno == ddl_chessisno.SelectedItem.Text && t.Branch_Name == branchname);
            pe.Vp_NetQuantity = 1;
            db.SaveChanges();

           ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "MessagePopUp", "<script language='JavaScript'>alert('Sales Return Sucessfully..!!'); </script>", false);
           btn_salereturn.Visible = false;
            return;
